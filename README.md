...
- **Web Worker 加速**：抖动算法在后台线程执行，4K 图片处理时 UI 不冻结
...
**核心特性：**
- **丰富的绘图工具**：
  - 铅笔工具：逐像素绘制，支持笔刷大小（1-20px）
  - 橡皮擦：清除像素
  - 填充工具：区域 flood fill 填充（扫描线算法，4K 下防 OOM）
  - 全局填充：一键替换画布中所有同色像素
  - 取色器：从画布拾取颜色
  - 直线/矩形/圆形工具：GPU 加速形状预览
  - 位移工具：移动图层内容
- **图层系统**：支持多图层、可见性切换、上下移动、复制、向下合并
- **蒙版功能**：图层蒙版裁剪上层内容
- **完整的调色板**：包含免费和付费颜色（63色），直观的颜色名称显示
- **撤销/重做**：支持多步操作历史回溯，4K 画布下快照 <50ms
- **自动描边**：内边线/外边线/内外边线三种模式
- **网格显示**：可切换显示像素网格辅助线
- **画布控制**：
  - 自由缩放（1x - 80x，丝滑滚轮+手势）
  - 平移导航
...
- **导入/导出**：
  - 支持导入外部图片作为底图或图层
  - 导出为 PNG（ImageData 方式，4K 导出 <2s）
  - 空白画布创建（预设 16×16 到 256×256，最大 10000×10000）
- **快捷键支持**：常用工具绑定键盘快捷键
- **移动端优化**：完整的触摸手势、双指缩放、单指绘画
- **4K 性能优化**：Uint32Array 扁平存储、脏矩形增量渲染、降采样缩略图
...
├── js/                         # JavaScript 文件目录
│   ├── app.js                  # 转换器主逻辑（含 Web Worker 内联代码）
│   ├── pixel-draw.js           # 绘画工具主逻辑（Uint32Array 数据模型）
│   └── pixel-draw.test.js      # 单元测试（85 个测试用例）
├── .github/memory/             # 项目记忆文档
...
- **图像处理**：Canvas API + OffscreenCanvas + Web Workers
- **数据模型**：Uint32Array 扁平像素存储（RGBA packed uint32）
- **渲染优化**：脏矩形增量重建 + requestAnimationFrame 批处理
- **算法实现**：纯 JavaScript 实现的多种抖动算法
- **响应式设计**：CSS Media Queries + Flexbox/Grid
- **测试**：85 个单元测试（纯函数 + 绘制操作 + 导出合成）
- **无依赖**：零第三方库依赖，轻量级设计
